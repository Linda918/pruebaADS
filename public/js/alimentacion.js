...
document.addEventListener("DOMContentLoaded", function () {
    const selectCantidad = document.getElementById("cantidad-comidas");
    const contenedorHorarios = document.getElementById("horarios-comidas");

    selectCantidad.addEventListener("change", function () {
        const cantidad = parseInt(this.value);
        contenedorHorarios.innerHTML = ""; // Limpiar campos anteriores
        ...
        for (let i = 1; i <= cantidad; i++) {
            ...
            label.textContent = `Selecciona la hora a la que te gustaría hacer tu comida ${i}:*`;

            const input = document.createElement("input");
            input.type = "time";
            input.className = "form-control";
            input.required = true;
            input.name = `hora-comida-${i}`;

            grupo.appendChild(label);
            grupo.appendChild(input);

            contenedorHorarios.appendChild(grupo);
        }
    });
});
